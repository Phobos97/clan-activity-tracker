...
@PluginDescriptor(
		name = "CAT",
		description = "Clan Activity Tracker. Developed by Phobos97, updated by c0du."
)
public class CATPlugin extends Plugin {
	private static final String BASE_DIRECTORY = RuneLite.RUNELITE_DIR + "/CAT/";

	private static final String[] HEADERS = { "rsn", "rank", "message count", "last message timestamp", "last seen online"};

	@SuppressWarnings("unused")
	@Provides
	CATConfig provideConfig(ConfigManager configManager) {
		return configManager.getConfig(CATConfig.class);
	}
	...
	public void onClanChannelChanged(ClanChannelChanged event) throws IOException {
		if (config.chatTrackType() != CATConfig.ChatTrackType.CLAN_CHAT && config.chatTrackType() != CATConfig.ChatTrackType.BOTH){
			return;
		}
		...
		try {
			...
			List<String> clanRsns = new ArrayList<>();
			List<String> updatedRsns = new ArrayList<>();
			...
			log.debug("CAT encountered an IO exception creating clan chat new player entry, likely since the .CSV file was open.");
		}
	}
	...
	public void onClanMemberJoined(ClanMemberJoined event) throws IOException {
		...
		try {
			...
			log.debug("CAT encountered an IO exception creating title entry, likely since the .CSV file was open.");
		}
	}
	...
	public void onFriendsChatMemberJoined(FriendsChatMemberJoined event) throws IOException {
		if (config.chatTrackType() != CATConfig.ChatTrackType.FRIEND_CHAT  && config.chatTrackType() != CATConfig.ChatTrackType.BOTH){
			return;
		}
		...
		try {
			...
			log.debug("CAT encountered an IO exception creating new friends chat player entry, likely since the .CSV file was open.");
		}
	}
	...
	public void onChatMessage(ChatMessage chatMessage) throws IOException {
		...
		if (chatTypesToTrack.contains(chatMessage.getType())) {
			...
			try {
				...
				log.debug("CAT encountered an IO exception creating message entry, likely since the .CSV file was open.");
			}
		}
	}
	...
	public void createFile(String Pathname) throws IOException {
		...
		if (!basedir.exists()) {
			if (!basedir.mkdir()) {
				log.error("Failed to create base directory: {}", BASE_DIRECTORY);
				throw new IOException("Could not create directory: " + BASE_DIRECTORY);
			}
		}
		...
		if (!file.exists()) {
			try (FileWriter outputfile = new FileWriter(file);
				 CSVPrinter printer = new CSVPrinter(outputfile, CSVFormat.DEFAULT
						 .withHeader(HEADERS))) {
				// Empty block intentionally, resources will be auto-closed
			}
		}
	}
